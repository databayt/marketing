  ...
  return (
    <div className="min-w-[300px] w-2/6" data-section="ready-to-build">
      <p className="font-medium mb-4 text-[oklch(1_0_0)]">
        {t.marketing.footer.readyToBuild}
      </p>
      <ExpandButton
        variant="white"
        href={`/${locale}/wizard`}
        className={`group flex items-center mb-6 cursor-pointer bg-[oklch(1_0_0)] text-[oklch(0.145_0_0)] border-[oklch(1_0_0)] w-fit md:w-fit ${isRTL ? "flex-row-reverse" : ""}`}
      >
        <span
        ...
        {isRTL ? (
          <ArrowLeft
            className={`order-1 transition-all duration-300 group-hover:order-2 group-hover:-translate-x-1 h-4 w-4 ${isRTL ? "mr-2 md:mr-2 group-hover:mr-0 group-hover:ml-2" : "ml-2 group-hover:ml-0 group-hover:mr-2"}`}
          />
        ) : (
